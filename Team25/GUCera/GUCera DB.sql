--ALTER DATABASE Gucera SET SINGLE_USER WITH ROLLBACK IMMEDIATE
...
--drop proc InstructorViewAssignmentsStudents
CREATE Proc InstructorViewAssignmentsStudents
@instrId int,
@cid int
As
Select S.sid ,S.cid,S.assignmentNumber, S.assignmenttype, S.grade
...
@cid int
As
Select f.number,f.comment ,f.numberOfLikes
From Feedback f inner join course c on c.id = f.cid
Where cid=@cid and instructorId =@instrId 
go

--exec ViewFeedbacksAddedByStudentsOnMyCourse 3, 1003
...
go
CREATE proc MycheckStudentEnrolledInCourse
@sid INT, @cid INT,
...
--exec checkStudentEnrolledInCourse 8,1, @returnValue
print @returnValue
...
go
--removed StudentTakeCourse.sid IS NULL because it checks if ANYBODY enrolled in a course, not a specific student and removed joins because it outputs duplicates
...
--drop proc courseInstructors
create proc courseInstructors
@id int
As
IF(EXISTS(SELECT * FROM Course WHERE id = @id))
Begin
Select Users.id,Users.firstName,Users.lastName from InstructorTeachCourse I inner join Course C on C.id = I.cid inner join Users on Users.id = I.insid 
where C.id = @id
End
Go

--exec courseInstructors 1
...
begin
	if (not exists(select * from studenttakecourse where sid = @sid and cid= @cid))
		begin
		INSERT INTO StudentTakeCourse (sid,cid,insid) VALUES (@sid,@cid,@instr)
		set @message = 'Enrolled!'
		end
	else
		set @message = 'You already take this course!' 
end
else
	set @message ='Student didnt take this course pre-requisite!'
end
else 
begin
	if (not exists(select * from studenttakecourse where sid = @sid and cid= @cid))
...
END
	...
	set @message = 'This instructor does not teach this course!'
END
go

/*
declare @message varchar(50)
exec enrollInCourse 1,1,3,@message
print @message
